  ...
  <div class="pl-8">
    <RouterLink to="/artist" class="cursor-pointer">
      ...
      >
        ...
        <img
          width="25"
          class="absolute z-40 right-0 bottom-0 pb-3 mr-3 contrast-150 brightness-110"
          src="/images/deezer-sound-icon.png"
        />
        <img class="rounded-md aspect-square" :src="slide.url" />
      </div>
      <div class="text-[#858590] text-left mt-2">
        <div class="text-sm hover:underline cursor-pointer text-white">
          {{ slide.song }}
        </div>
        <div class="text-[13px] flex hover:underline pt-0.5">
          by {{ slide.by }}
        </div>
        <div class="text-[11px] pt-0.5">Released on {{ slide.releasedOn }}</div>
      </div>
    </RouterLink>
  </div>
</template>
